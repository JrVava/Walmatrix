...
import { findLatestDate, getPnlByItem } from '../modules/utils';
// import { group } from 'console';

@JsonController('/pnl-dashboard')
@UseBefore(loggingMiddleware)
export class PNLDashboardController extends DataSourceConnection {
    ...
    async index(@Body() dataObj, @Res() res: Response) {
        ...
        const userSettingRepository =
            await publicConnection.getRepository(UsersSettings);
        const getReconManagement = await userSettingRepository
            .createQueryBuilder('userSetting')
            .select('rm.available_date', 'available_date')
            .leftJoin(
                'recon_management',
                'rm',
                'rm.user_setting_id = userSetting.id',
            )
        ...
        if (id.length > 0) {
            ...
            const orderQuery = `SELECT
                ...
                ORDER BY
                    gs.date ASC;`;
            const orderResults = await publicConnection.query(orderQuery);
            ...
            const wfsQuery = `select
                ...
                order by
                    gs.date asc `;
            const wfsResults = await publicConnection.query(wfsQuery);
            const disputesQuery = `select
                    ...
                    order by
                        gs.date asc `;
            const disputeResults = await publicConnection.query(disputesQuery);
            ...
            const commissionResult =
                await publicConnection.query(commissionQuery);
            ...
            const returnQuery = `select
                ...
                order by
                    gs.date asc `;
            const returnResult = await publicConnection.query(returnQuery);
            ...
            const ad_spendsQuery = `select
                ...
                order by
                    gs.date ASC`;
            const ad_spendResults =
                await publicConnection.query(ad_spendsQuery);
            ...
            const cogsResults = await publicConnection.query(cogsQuery);
            ...
            order by
                gs.date asc `;
            const shippingResults = await publicConnection.query(shippingQuery);
            ...
            const capFeesResults = await publicConnection.query(capFeesQuery);
            ...
            const pnlByDate = dateResults.map((date) => {
                ...
                const matchingCogsResults = cogsResults.find((data) => {
                    if (data.date === date.date) return data.total_cogs_amount;
                });
                ...
                const matchingCommissionResult = commissionResult.find(
                    (data) => {
                        if (data.date === date.date)
                ...
                const matchingWfsResult = wfsResults.find((data) => {
                    if (data.date === date.date) return data.total_wfs_amount;
                });

                const matchingAccountLevelfees = accountLevelfeesForGraph.find(
                    (data) => {
                        if (data.date === date.date)
            ...
            const totalPNL = pnlByDate
                .map((x) => x.pnl)
                .reduce((a, b) => a + b);
            const differenceInDays = moment(dataObj.startDate).diff(
                moment(dataObj.endDate),
                'days',
            );
            ...
            const item_fees = await this.getitemLevelFee(
                schema_name,
                startDate,
                endDate,
                id,
            ...
            const comparison_item_fees = await this.getitemLevelFee(
                schema_name,
                comparison_startDate,
                comparison_endDate,
                id,
            );
            const account_fees = await this.getAccountLevelFee(
                schema_name,
                startDate,
                endDate,
                id,
            );
            const comparison_account_fees = await this.getAccountLevelFee(
                schema_name,
                comparison_startDate,
                comparison_endDate,
                id,
            ...
            const accountFeesTotal = Object.values(
    ...
    @Post('/pnl-by-item')
    async pnlByItem(@Body() dataObj, @Res() res: Response) {
        const id = dataObj.store_id;
        const filter = dataObj.filter;

        const getSchema = await findSchemaNameWithArray(id);
        if (id.length > 0) {
            const startDate = moment(dataObj.startDate);
            const endDate = moment(dataObj.endDate);
            const comparison_startDate = moment(dataObj.comparison_startDate);
            const comparison_endDate = moment(dataObj.comparison_endDate);

            const schema_name = getSchema.schemaName;
            const productRepository = (
                await this.connection(schema_name)
            ).getRepository(Products);
            const data = await getPnlByItem(
                productRepository,
                startDate,
                endDate,
                id,
            ...
            const comparison_data = await getPnlByItem(
                productRepository,
                comparison_startDate,
                comparison_endDate,
                id,
                    ...
                    if (item.item_id === comparison.item_id) {
                        ...
                        result.push({
                            ...
                            organic_sales_percentage:
                                item.organic_sales_percentage * 100,
                            comparison_organic_sales_percentage:
                                comparison.organic_sales_percentage * 100,
                            roas: item.roas,
                            ...
                            total_wfs_amount: item.total_wfs_amount,
                            comparison_total_wfs_amount:
                                comparison.total_wfs_amount,
                            total_dispute_amount: item.total_dispute_amount,
                            comparison_total_dispute_amount:
                                comparison.total_dispute_amount,
                            total_commission_amount:
                                item.total_commission_amount,
                            comparison_total_commission_amount:
                                comparison.total_commission_amount,
                            total_return_amount: item.total_return_amount,
                            comparison_total_return_amount:
                                comparison.total_return_amount,
                            shipping_total: item.shipping_total,
                            comparison_shipping_total:
                                comparison.shipping_total,
                            cogs_total: item.cogs_total,
    ...
    async barChart(id, startDate, endDate, schema_name, daysCount) {
        ...
        if (id.length > 0) {
            const subqueryAccountFees = await reconRepository
                .createQueryBuilder('recon')
                .select('COALESCE(SUM(recon.amount), 0)', 'total_amount')
                .addSelect(
                    "TO_CHAR(recon.transaction_posted_timestamp, 'YYYY-MM-DD')",
                    'date',
                )
            ...
            const inboundTransportationFee = await subqueryAccountFees
                .andWhere(
            ...
            const storageFee = await subqueryAccountFees
                .andWhere(
                    'transaction_description = :transaction_description',
            ...
            const lostInventory = await subqueryAccountFees
                .andWhere(
                    'transaction_description = :transaction_description',
            ...
            const foundInventory = await subqueryAccountFees
                .andWhere(
                    'transaction_description = :transaction_description',
            ...
            const refund = await subqueryAccountFees
                .andWhere(
                    'transaction_description = :transaction_description',
            ...
            const damageInWarehouse = await subqueryAccountFees
                .andWhere(
                    'transaction_description = :transaction_description',
            ...
            const rc_InventoryDisposalFee = await subqueryAccountFees
                .andWhere(
                    'transaction_description = :transaction_description',
                ...
                .groupBy('recon.transaction_posted_timestamp')
                .getRawMany();
            const prepServiceFee = await subqueryAccountFees
                .andWhere(
                    'transaction_description = :transaction_description',
            ...
            const inventoryTransferFee = await subqueryAccountFees
                .andWhere(
                    'transaction_description = :transaction_description',
            ...
            const rc_InventoryRTVFee = await subqueryAccountFees
                .andWhere(
                    'transaction_description = :transaction_description',
                ...
                .groupBy('recon.transaction_posted_timestamp')
                .getRawMany();
            const ad_logo = await advertiserRepository
            ...
            for (let index = 0; index < daysCount; index++) {
                ...
                if (
                    inboundTransportationFee.length > 0 &&
                    inboundTransportationFee != null
                ) {
                    const matchingInboundRecord = inboundTransportationFee.find(
                        (record) => {
                            return formattedDate === record.date;
                        },
                    );
                    if (matchingInboundRecord) {
                        inBoundTransportationFeeArray[index] = parseFloat(
                            matchingInboundRecord.total_amount,
                        );
                    } else {
                ...
                if (storageFee.length > 0 && storageFee != null) {
                    ...
                    if (matchingStorageRecord) {
                        storageFeesArray[index] = parseFloat(
                            matchingStorageRecord.total_amount,
                        );
                    } else {
                ...
                if (lostInventory.length > 0 && lostInventory != null) {
                    const matchingLostInvntoryRecord = lostInventory.find(
                        (record) => {
                            return formattedDate === record.date;
                        },
                    );
                    if (matchingLostInvntoryRecord) {
                        lostinventoryArray[index] = parseFloat(
                            matchingLostInvntoryRecord.total_amount,
                        );
                    } else {
                ...
                if (foundInventory.length > 0 && foundInventory != null) {
                    const matchingFoundInventoryRecord = foundInventory.find(
                        (record) => {
                            return formattedDate === record.date;
                        },
                    );
                    if (matchingFoundInventoryRecord) {
                        foundInventoryArray[index] = parseFloat(
                            matchingFoundInventoryRecord.total_amount,
                        );
                    } else {
                ...
                if (refund.length > 0 && refund != null) {
                    ...
                    if (matchingRefundRecord) {
                        refundArray[index] = parseFloat(
                            matchingRefundRecord.total_amount,
                        );
                    } else {
                        refundArray[index] = 0;
                    }
                ...
                if (damageInWarehouse.length > 0 && damageInWarehouse != null) {
                    const matchingDamageInWarehouseRecord =
                        damageInWarehouse.find((record) => {
                            return formattedDate === record.date;
                        });
                    if (matchingDamageInWarehouseRecord) {
                        damageInWareHouseArray[index] = parseFloat(
                            matchingDamageInWarehouseRecord.total_amount,
                        );
                    } else {
                ...
                if (
                    rc_InventoryDisposalFee.length > 0 &&
                    rc_InventoryDisposalFee != null
                ) {
                    const matchingRC_InventoryDisposalFeeRecord =
                        rc_InventoryDisposalFee.find((record) => {
                            return formattedDate === record.date;
                        });
                    if (matchingRC_InventoryDisposalFeeRecord) {
                        rc_InventoryDisposalFeeArray[index] = parseFloat(
                            matchingRC_InventoryDisposalFeeRecord.total_amount,
                        );
                    } else {
                ...
                if (prepServiceFee.length > 0 && prepServiceFee != null) {
                    const matchingPrepServiceFeeRecord = prepServiceFee.find(
                        (record) => {
                            return formattedDate === record.date;
                        },
                    );
                    if (matchingPrepServiceFeeRecord) {
                        prepServiceFeeArray[index] = parseFloat(
                            matchingPrepServiceFeeRecord.total_amount,
                        );
                    } else {
                ...
                if (
                    inventoryTransferFee.length > 0 &&
                    inventoryTransferFee != null
                ) {
                    const matchingInventoryTransferFeeRecord =
                        inventoryTransferFee.find((record) => {
                            return formattedDate === record.date;
                        });
                    if (matchingInventoryTransferFeeRecord) {
                        inventoryTransferFeeArray[index] = parseFloat(
                            matchingInventoryTransferFeeRecord.total_amount,
                        );
                    } else {
                ...
                if (
                    rc_InventoryRTVFee.length > 0 &&
                    rc_InventoryRTVFee != null
                ) {
                    const matchingRC_InventoryRTVFeeRecord =
                        rc_InventoryRTVFee.find((record) => {
                            return formattedDate === record.date;
                        });
                    if (matchingRC_InventoryRTVFeeRecord) {
                        rc_InventoryRTVFeeArray[index] = parseFloat(
                            matchingRC_InventoryRTVFeeRecord.total_amount,
                        );
                    } else {
                ...
                if (ad_logo.length > 0 && ad_logo != null) {
                    ...
                    if (matchingAD_logoRecord) {
                        ad_logoArray[index] = parseFloat(
                            matchingAD_logoRecord.total_ad_spend,
                        );
                    } else {
    ...
    async getAccountLevelFee(schema_name, startDate, endDate, id) {
        const reconRepository = (
            await this.connection(schema_name)
        ).getRepository(Recon);
        const advertiserRepository = (
        ...
        const reviewAccelerator = await subqueryAccountFees
            .andWhere('transaction_description = :transaction_description', {
                transaction_description: 'Review Accelerator',
            })
            .getRawOne();

        const WFSInventoryRemovalOrder = await subqueryAccountFees
            .andWhere('transaction_description = :transaction_description', {
        ...
        const account_fees = {
            inboundTransportationFee:
                ...
                inboundTransportationFee.total_amount != null
                    ...
                    : 0,
            storageFee:
                storageFee && storageFee.total_amount != null
                    ? parseFloat(storageFee.total_amount)
                    : 0,
            lostInventory:
                lostInventory && lostInventory.total_amount != null
                    ? parseFloat(lostInventory.total_amount)
                    : 0,
            foundInventory:
                foundInventory && foundInventory.total_amount != null
                    ? parseFloat(foundInventory.total_amount)
                    : 0,
            refund:
                refund && refund.total_amount != null
                    ? parseFloat(refund.total_amount)
                    : 0,
            damageInWarehouse:
                damageInWarehouse && damageInWarehouse.total_amount != null
            ...
            rc_InventoryDisposalFee:
                ...
                rc_InventoryDisposalFee.total_amount != null
                    ...
                    : 0,
            prepServiceFee:
                prepServiceFee && prepServiceFee.total_amount != null
            ...
            inventoryTransferFee:
                ...
                inventoryTransferFee.total_amount != null
                    ...
                    : 0,
            rc_InventoryRTVFee:
                rc_InventoryRTVFee && rc_InventoryRTVFee.total_amount != null
                    ? parseFloat(rc_InventoryRTVFee.total_amount)
                    : 0,
            reviewAccelerator:
                reviewAccelerator && reviewAccelerator.total_amount != null
                    ? parseFloat(reviewAccelerator.total_amount)
                    : 0,
            ad_logo:
                ad_logo && ad_logo.total_ad_spend != null
                    ? parseFloat(ad_logo.total_ad_spend) * -1
                    : 0,
    ...
    async getitemLevelFee(schema_name, startDate, endDate, id) {
        ...
        const subQueryOrder = await ordersRepository
        ...
        const sale = await subQueryOrder.getRawOne();

        const returnBaseQuery = await reconRepository
            .createQueryBuilder('recon')
            .select('COALESCE(SUM(recon.amount), 0)', 'total_amount')
            ...
            .andWhere(
                'recon.transaction_description IN (:...transaction_description)',
                {
        ...
        const returnItemFees = await returnBaseQuery.getRawOne();

        const subQueryAdspend = await advertiserRepository
            .createQueryBuilder('advertisers')
            .select('sum(advertisers.ad_spend)', 'total_ad_spend')
            .where('advertisers.store_id IN (:...values)', { values: id })
            .andWhere('advertisers.date BETWEEN :startDate AND :endDate', {
        ...
        const ad_spendItemfees = await subQueryAdspend.getRawOne();

        const commissionBaseQuery = await reconRepository
            .createQueryBuilder('recon')
            .select('COALESCE(SUM(recon.amount), 0)', 'total_amount')
        ...
        const commissionItemFees = await commissionBaseQuery.getRawOne();
        ...
        // .getRawMany();
        const wfsFulFillmentItemFees = await wfsBaseQuery.getRawOne();

        const subQueryCogs = await ordersRepository
            .createQueryBuilder('o')
        ...
        const cogItemFess = await subQueryCogs.getRawOne();
        ...
        const disputesItemfess = await disputesBaseQuery.getRawOne();

        const subqueryShipping = await reconRepository
            .createQueryBuilder('recon')
            .select('COALESCE(SUM(recon.amount), 0)', 'total_amount')
            .where('recon.transaction_type = :transaction_type', {
            ...
            .andWhere(
                'recon.transaction_description = :transaction_description',
                {
                    transaction_description:
        ...
        const sippingItemFees = await subqueryShipping.getRawOne();

        const subQueryCap = await reconRepository
            .createQueryBuilder('recon')
            .select('COALESCE(SUM(recon.amount), 0)', 'total_amount')
        ...
        const capItemFees = await subQueryCap.getRawOne();

        const item_fees = {
            sales:
                sale && sale.total_order_amount != null
                    ? parseFloat(sale.total_order_amount)
                    : 0,
            returns:
                returnItemFees && returnItemFees.total_amount != null
                    ? parseFloat(returnItemFees.total_amount)
                    : 0,
            ad_spend:
                ad_spendItemfees && ad_spendItemfees.total_ad_spend != null
                    ? parseFloat(ad_spendItemfees.total_ad_spend)
                    : 0,
            commission:
                commissionItemFees && commissionItemFees.total_amount != null
                    ? parseFloat(commissionItemFees.total_amount)
                    : 0,
            wfs_fulFillment:
                wfsFulFillmentItemFees &&
                wfsFulFillmentItemFees.total_amount != null
                    ? parseFloat(wfsFulFillmentItemFees.total_amount)
                    : 0,
            cogItem:
                cogItemFess && cogItemFess.cogs_total != null
                    ? parseFloat(cogItemFess.cogs_total)
                    : 0,
            disputes:
                disputesItemfess && disputesItemfess.total_amount != null
                    ? parseFloat(disputesItemfess.total_amount)
                    : 0,
            shipping_fees:
                sippingItemFees && sippingItemFees.total_amount != null
                    ? parseFloat(sippingItemFees.total_amount)
                    : 0,
            cap:
                capItemFees && capItemFees.total_amount != null
    ...
    async pnlByItemData(
        ...
        const queryBuilder = await productRepository
            ...
            .addSelect(
                'coalesce(SUM(oc.total_amount_sale),0)',
                'total_amount_sale',
            )
            ...
            .leftJoin(
                (subQuery) => {
                    return subQuery
                        ...
                        .andWhere(
                            'rcap.amount_type = :cap_shipping_amountType',
                            {
                                cap_shipping_amountType:
    ...
    async accountLevelFeesForGraph(schema_name, id, startDate, endDate) {
        const publicConnection = await this.publicConnection();
        const dateQuery = `SELECT TO_CHAR(generate_series('${startDate}'::date, '${endDate}'::date, '1 day'::interval), 'YYYY-MM-DD') AS date`;
        const dateResults = await publicConnection.query(dateQuery);
        ...
        const inboundTransportationFeeResult = await publicConnection.query(
            inboundTransportationFeeQuery,
        );
        ...
        order by gs.date asc
        `;
        const storageFeesResult = await publicConnection.query(storageFeeQuery);
        ...
        const lostInventoryFeesResult =
            await publicConnection.query(lostInventoryQuery);
        ...
        order by gs.date asc
        `;
        const foundInventoryFeesResult =
            await publicConnection.query(foundInvetoryQuery);
        ...
        const refundFeesResult = await publicConnection.query(refundQuery);
        ...
        const damageInWarehouseResult = await publicConnection.query(
            damageInWarehouseQuery,
        );
        ...
        const rc_InventoryDisposalFeeQuery = `
            ...
            order by
                gs.date asc
        `;
        const rc_InventoryDisposalFeeResult = await publicConnection.query(
            rc_InventoryDisposalFeeQuery,
        );
        ...
        const prepServiceFeeResult =
            await publicConnection.query(prepServiceFeeQuery);
        ...
        const inventoryTransferFeeResult = await publicConnection.query(
            inventoryTransferFeeQuery,
        );
        ...
        order by
            gs.date asc
        `;
        const rc_InventoryRTVFeeResult = await publicConnection.query(
            rc_InventoryRTVFeeQuery,
        );
        ...
        const ad_logoQuery = `
            ...
            order by
                gs.date asc
        `;
        const ad_logoResult = await publicConnection.query(ad_logoQuery);
        ...
        const reviewAcceleratorResult = await publicConnection.query(
            reviewAcceleratorQuery,
        );
        ...
        const WFSInventoryRemovalOrderResult = await publicConnection.query(
            WFSInventoryRemovalOrderQuery,
        );

        const accountLevelFeesByDate = dateResults.map((date) => {
            const matchingInBoundRecords = inboundTransportationFeeResult.find(
                (data) => {
            ...
            const matchingLostInventoryFees = lostInventoryFeesResult.find(
                (data) => {
                    if (data.date === date.date) return data.total_amount;
                },
            );

            const matchingFoundInventoryFees = foundInventoryFeesResult.find(
                (data) => {
                    if (data.date === date.date) return data.total_amount;
                },
            );
            ...
            const matchingDamageInWarehouse = damageInWarehouseResult.find(
                (data) => {
                    if (data.date === date.date) return data.total_amount;
                },
            );
            ...
            const matchingRC_InventoryDisposalFee =
                rc_InventoryDisposalFeeResult.find((data) => {
                    if (data.date === date.date) return data.total_amount;
                });
            ...
            const matchingInventoryTransferFee =
                inventoryTransferFeeResult.find((data) => {
                    if (data.date === date.date) return data.total_amount;
                });

            const matchingRC_InventoryRTVFee = rc_InventoryRTVFeeResult.find(
                (data) => {
                    if (data.date === date.date) return data.total_amount;
                },
            );

            const matchingReviewAccelerator = reviewAcceleratorResult.find(
                (data) => {
                    if (data.date === date.date) return data.total_amount;
                },
            );
            ...
            const matchingWFSInventoryRemovalOrder =
                WFSInventoryRemovalOrderResult.find((data) => {
                    if (data.date === date.date) return data.total_amount;
                });
